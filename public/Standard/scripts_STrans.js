...
function refreshGrid() {
    fetch('/StandTransport/projectsHTML')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function searchProject() {
    ...
    fetch(`/StandTransport/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    .then(data => {
        const projectUrl = `/StandTransport/display?refId=${data.refId}`;
...
function viewDocumentation(refId) {
    fetch(`/StandTransport/display_documentation?refId=${refId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          link.href = `/StandTransport/get-documentation/${doc}`;
