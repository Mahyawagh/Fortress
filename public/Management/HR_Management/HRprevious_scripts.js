...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchProjectList() {
      fetch('/HRprevioussearch/project_list')
      .then(response => {
        if (!response.ok) {
  ...
  function searchProject() {
    ...
    fetch(`/HRprevioussearch/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    if (searchResultsContainer) {
        ...
        if (data.projects && data.projects.length > 0) {
            ...
            data.projects.forEach(project => {
                ...
                link.href = `/HRprevioussearch/display?empId=${project.emp_id}`;
        ...
        if (data.files && data.files.length > 0) {
            ...
            data.files.forEach(file => {
                const link = document.createElement('a');
                link.href = `/HRprevioussearch/get-image/${file.filename}`;
...
function viewDocumentation(empId) {
    fetch(`/HRprevioussearch/display_documentation?empId=${empId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          const link = document.createElement('a');
          link.href = `/HRprevioussearch/get-documentation/${doc}`;
