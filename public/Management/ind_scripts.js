...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchProjectList() {
      fetch('/manaindustrial/project_list')
      .then(response => {
          if (!response.ok) {
  ...
  function searchProject() {
    ...
    fetch(`/manaindustrial/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function displaySearchResults(data) {
  ...
  if (searchResultsContainer) {
      ...
      if (data.projects && data.projects.length > 0) {
          ...
          data.projects.forEach(project => {
              ...
              link.href = `/manaindustrial/display?projectId=${project.project_id}`;
      ...
      if (data.files && data.files.length > 0) {
          ...
          data.files.forEach(file => {
              const link = document.createElement('a');
              link.href = `/manaindustrial/get-image/${file.filename}`;
  ...
  function viewPhotographs(projectId) {
      fetch(`/manaindustrial/display_project?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
  ...
  function viewDocumentation(projectId) {
      fetch(`/manaindustrial/display_documentation?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
    ...
    function viewReport(projectId) {
      fetch(`/manaindustrial/display_report?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
        ...
        .then(data => {
          ...
          report.forEach(report => {
            const listItem = document.createElement('li');
            const link = document.createElement('a');
            link.href = `/manaindustrial/get-report/${report}`;
  ...
  function viewPresentation(projectId) {
    fetch(`/manaindustrial/display_presentation?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        presentation.forEach(presentation => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/manaindustrial/get-presentation/${presentation}`;
  ...
  function viewOther(projectId) {
    fetch(`/manaindustrial/display_other?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        other.forEach(other => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/manaindustrial/get-other/${other}`;
