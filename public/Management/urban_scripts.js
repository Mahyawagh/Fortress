...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function searchProject() {
    ...
    fetch(`/manaurban/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function displaySearchResults(data) {
  ...
  if (searchResultsContainer) {
      ...
      if (data.projects && data.projects.length > 0) {
          ...
          data.projects.forEach(project => {
              ...
              link.href = `/manaurban/display?projectId=${project.project_id}`;
      ...
      if (data.files && data.files.length > 0) {
          ...
          data.files.forEach(file => {
              const link = document.createElement('a');
              link.href = `/manaurban/get-image/${file.filename}`;
  ...
  function viewPhotographs(projectId) {
      fetch(`/manaurban/display_project?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
  ...
  function viewDocumentation(projectId) {
      fetch(`/manaurban/display_documentation?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
    ...
    function viewReport(projectId) {
      fetch(`/manaurban/display_report?projectId=${projectId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
        ...
        .then(data => {
          ...
          report.forEach(report => {
            const listItem = document.createElement('li');
            const link = document.createElement('a');
            link.href = `/manaurban/get-report/${report}`;
  ...
  function viewPresentation(projectId) {
    fetch(`/manaurban/display_presentation?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        presentation.forEach(presentation => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/manaurban/get-presentation/${presentation}`;
  ...
  function viewOther(projectId) {
    fetch(`/manaurban/display_other?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        other.forEach(other => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/manaurban/get-other/${other}`;
