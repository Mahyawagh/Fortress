...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchProjectList() {
      fetch('/otherurbansearch/project_list')
      .then(response => {
        if (!response.ok) {
  ...
  function searchProject() {
    ...
    fetch(`/otherurbansearch/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    if (searchResultsContainer) {
        ...
        if (data.projects && data.projects.length > 0) {
            ...
            data.projects.forEach(project => {
                ...
                link.href = `/otherurbansearch/display?refId=${project.ref_id}`;
        ...
        if (data.files && data.files.length > 0) {
            ...
            data.files.forEach(file => {
                const link = document.createElement('a');
                link.href = `/otherurbansearch/get-image/${file.filename}`;
...
function viewDocumentation(refId) {
    fetch(`/otherurbansearch/display_documentation?refId=${refId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          const link = document.createElement('a');
          link.href = `/otherurbansearch/get-documentation/${doc}`;
