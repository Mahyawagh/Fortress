...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchProjectList() {
      fetch('/othertourismsearch/project_list')
      .then(response => {
        if (!response.ok) {
  ...
  function searchProject() {
    ...
    fetch(`/othertourismsearch/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    if (searchResultsContainer) {
        ...
        if (data.projects && data.projects.length > 0) {
            ...
            data.projects.forEach(project => {
                ...
                link.href = `/othertourismsearch/display?refId=${project.ref_id}`;
        ...
        if (data.files && data.files.length > 0) {
            ...
            data.files.forEach(file => {
                const link = document.createElement('a');
                link.href = `/othertourismsearch/get-image/${file.filename}`;
...
function viewDocumentation(refId) {
    fetch(`/othertourismsearch/display_documentation?refId=${refId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          const link = document.createElement('a');
          link.href = `/othertourismsearch/get-documentation/${doc}`;
