...
function refreshGrid() {
    fetch('/OtherTransport/projectsHTML')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function searchProject() {
    ...
    fetch(`/OtherTransport/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    .then(data => {
        const projectUrl = `/OtherTransport/display?refId=${data.refId}`;
...
function viewDocumentation(refId) {
    fetch(`/OtherTransport/display_documentation?refId=${refId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          link.href = `/OtherTransport/get-documentation/${doc}`;
