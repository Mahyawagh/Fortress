...
function refreshGrid() {
    fetch('/OtherTourism/projectsHTML')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function searchProject() {
    ...
    fetch(`/OtherTourism/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
    ...
    .then(data => {
        const projectUrl = `/OtherTourism/display?refId=${data.refId}`;
...
function viewDocumentation(refId) {
    fetch(`/OtherTourism/display_documentation?refId=${refId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        documentation.forEach(doc => {
          ...
          link.href = `/OtherTourism/get-documentation/${doc}`;
