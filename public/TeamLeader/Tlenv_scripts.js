...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function searchProject() {
    ...
    fetch(`/Tlenv/search?searchInput=${searchInput}`)
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
...
function displaySearchResults(data) {
  ...
  if (searchResultsContainer) {
      ...
      if (data.projects && data.projects.length > 0) {
          ...
          data.projects.forEach(project => {
              ...
              link.href = `/Tlenv/display?projectId=${project.project_id}`;
      ...
      if (data.files && data.files.length > 0) {
          ...
          data.files.forEach(file => {
              const link = document.createElement('a');
              link.href = `/Tlenv/get-image/${file.filename}`;
  ...
  function viewTendersight(projectId) {
    fetch(`/Tlenv/display_tendersight?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        tendersight.forEach(tendersight => {
            ...
            const link = document.createElement('a');
          link.href = `/Tlenv/get-tendersight/${tendersight}`;
  ...
  function viewTentativeletter(projectId) {
    fetch(`/Tlenv/display_tentativeletter?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        tentativeletter.forEach(tentativeletter => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/Tlenv/get-tentativeletter/${tentativeletter}`;
  ...
  function viewBidsfinancial(projectId) {
    fetch(`/Tlenv/display_bidsfinancial?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        bidsfinacial.forEach(bidsfinacial => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/Tlenv/get-bidsfinacial/${bidsfinacial}`;
  ...
  function viewBidstechnical(projectId) {
    fetch(`/Tlenv/display_bidstechnical?projectId=${projectId}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(data => {
        ...
        bidstechnical.forEach(bidstechnical => {
          const listItem = document.createElement('li');
          const link = document.createElement('a');
          link.href = `/Tlenv/get-bidstechnical/${bidstechnical}`;
