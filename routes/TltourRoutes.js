    ...
    db.query('SELECT * FROM Tourism WHERE project_id = ?', [projectId], (err, result) => {
        if (err) {
            ...
            if (result.length > 0) {
                ...
                htmlContent += `<p><strong>Photographs:</strong> <a href="/Tltour/display_project?projectId=${project.project_id}" target="_blank">View Photographs</a></p>`;
                htmlContent += `<p><strong>Documentation:</strong> <a href="/Tltour/display_documentation?projectId=${project.project_id}" target="_blank">View Documentation</a></p>`;
                htmlContent += `<p><strong>Reports:</strong> <a href="/Tltour/display_report?projectId=${project.project_id}" target="_blank">View Report</a></p>`;
                htmlContent += `<p><strong>Presentation:</strong> <a href="/Tltour/display_presentation?projectId=${project.project_id}" target="_blank">View Presentation</a></p>`;
                htmlContent += `<p><strong>Other:</strong> <a href="/Tltour/display_other?projectId=${project.project_id}" target="_blank">View Other </a></p>`;
...
function generatePhotographsHTML(photographs) {
    ...
    photographs.forEach(photo => {
      html += `<img src="/Tltour/get-image/${photo}" alt="${photo}">`;
    });
    html += "</div>";
    return html;
  }
...
function generateDocumentationHTML(documentation) {
    ...
    documentation.forEach(doc => {
        html += `<p><a href="/Tltour/get-image/${doc}" target="_blank">${doc}</a></p>`;
    });
    return html;
}
...
function generateReportHTML(report) {
    ...
    report.forEach(report => {
        html += `<p><a href="/Tltour/get-image/${report}" target="_blank">${report}</a></p>`;
    });
    return html;
}
function generatePresentationHTML(presentation) {
    ...
    presentation.forEach(presentation => {
        html += `<p><a href="/Tltour/get-image/${presentation}" target="_blank">${presentation}</a></p>`;
    });
    return html;
}
...
function generateOtherHTML(other) {
    ...
    other.forEach(other => {
        html += `<p><a href="/Tltour/get-image/${other}" target="_blank">${other}</a></p>`;
    });
    return html;
}
