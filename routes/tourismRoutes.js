...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS Tourism (
  ...
  const createPhotographsTableQuery = `CREATE TABLE IF NOT EXISTS Tour_Photographs (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    photograph BLOB,
    FOREIGN KEY (project_id) REFERENCES Tourism(project_id)
  )`;
  db.query(createPhotographsTableQuery, err => {
    if (err) throw err;
    console.log('Tour_Photographs table created');
  });

  const createDocumentationTableQuery = `CREATE TABLE IF NOT EXISTS Tour_Documentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    documentation BLOB,
    FOREIGN KEY (project_id) REFERENCES Tourism(project_id)
  ...
  db.query(createDocumentationTableQuery, err => {
    if (err) throw err;
    console.log('Tour_Documentation table created');
  });
  const createReportTableQuery = `CREATE TABLE IF NOT EXISTS Tour_Report (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    report BLOB,
    FOREIGN KEY (project_id) REFERENCES Tourism(project_id)
  ...
  db.query(createReportTableQuery, err => {
    if (err) throw err;
    console.log('Tour_Report table created');
  });
  const createPresentationTableQuery = `CREATE TABLE IF NOT EXISTS Tour_Presentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    presentation BLOB,
    FOREIGN KEY (project_id) REFERENCES Tourism(project_id)
  ...
  db.query(createPresentationTableQuery, err => {
    if (err) throw err;
    console.log('Tour_Presentation table created');
  });
  const createOtherTableQuery = `CREATE TABLE IF NOT EXISTS Tour_Other (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    other BLOB,
    FOREIGN KEY (project_id) REFERENCES Tourism(project_id)
  ...
  db.query(createOtherTableQuery, err => {
    if (err) throw err;
    console.log('Tour_Other table created');
  });
...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    ...
    const modifiedFilename = `Tour${projectId}_${Date.now()}_${file.fieldname}${extension}`; // Generate modified filename
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'Tourism')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'Tourism', 'tourism.html'));
});
...
router.post('/upload', upload.fields([
  ...
  const formattedProjectName = `Tour${project_name ? '_' + project_name : ''}`; // Update formatted project name
  const sql = `INSERT INTO Tourism (project_name, year) VALUES (?, ?)`;
  db.query(sql, [formattedProjectName, year], (err, result) => {
    if (err) {
      ...
      const formattedProjectName = `Tour${projectId}_${project_name}`;
    //console.log(formattedProjectName); 
      ...
      photographs.forEach(photo => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', `Tour${projectId}_${photo.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertPhotographQuery = `INSERT INTO Tour_Photographs (project_id, photograph) VALUES (?, ?)`;
        db.query(insertPhotographQuery, [projectId, `Tour${projectId}_${photo.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      documentation.forEach(doc => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', `Tour${projectId}_${doc.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertDocumentationQuery = `INSERT INTO Tour_Documentation (project_id, documentation) VALUES (?, ?)`;
        db.query(insertDocumentationQuery, [projectId, `Tour${projectId}_${doc.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      report.forEach(report => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', `Tour${projectId}_${report.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertReportQuery = `INSERT INTO Tour_Report (project_id, report) VALUES (?, ?)`;
        db.query(insertReportQuery, [projectId, `Tour${projectId}_${report.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      presentation.forEach(presentationFile => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', `Tour${projectId}_${presentationFile.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertPresentationQuery = `INSERT INTO Tour_Presentation (project_id, presentation) VALUES (?, ?)`;
        db.query(insertPresentationQuery, [projectId, `Tour${projectId}_${presentationFile.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      other.forEach(other => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', `Tour${projectId}_${other.originalname}`);
        fs.renameSync(oldPath, newPath);
        
        const insertOtherQuery = `INSERT INTO Tour_Other (project_id, other) VALUES (?, ?)`;
        db.query(insertOtherQuery, [projectId, `Tour${projectId}_${other.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT t.project_id, t.project_name, t.year, p.photograph, d.documentation, r.report, pr.presentation, o.other FROM Tourism t LEFT JOIN Tour_Photographs p ON t.project_id = p.project_id LEFT JOIN Tour_Documentation d ON t.project_id = d.project_id LEFT JOIN Tour_Report r ON t.project_id = r.project_id LEFT JOIN Tour_Presentation pr ON t.project_id = pr.project_id LEFT JOIN Tour_Other o ON t.project_id = o.project_id', (err, result) => {
    if (err) {
...
router.get('/get-image/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, '..', 'uploads', 'Tourism', filename);
...
router.get('/display', (req, res) => {
  ...
  db.query('SELECT * FROM Tourism WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Photographs:</strong> <a href="/tourism/display_project?projectId=${project.project_id}" target="_blank">View Photographs</a></p>`;
        htmlContent += `<p><strong>Documentation:</strong> <a href="/tourism/display_documentation?projectId=${project.project_id}" target="_blank">View Documentation</a></p>`;
        htmlContent += `<p><strong>Report:</strong> <a href="/tourism/display_report?projectId=${project.project_id}"target="_blank">View Report</a></p>`;
        htmlContent += `<p><strong>Presentation:</strong> <a href="/tourism/display_presentation?projectId=${project.project_id}"target="_blank">View Presentation </a></p>`;
        htmlContent += `<p><strong>Other:</strong> <a href="/tourism/display_other?projectId=${project.project_id}"target="_blank">View Other</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM Tourism WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
router.get('/display_project', (req, res) => {
  ...
  db.query('SELECT photograph FROM Tour_Photographs WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_documentation', (req, res) => {
  ...
  db.query('SELECT documentation FROM Tour_Documentation WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_report', (req, res) => {
  ...
  db.query('SELECT report FROM Tour_Report WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_presentation', (req, res) => {
  ...
  db.query('SELECT presentation FROM Tour_Presentation WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_other', (req, res) => {
  ...
  db.query('SELECT other FROM Tour_Other WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
function generatePhotographsHTML(photographs) {
  ...
  `;
  photographs.forEach(photo => {
    html += `<img src="/tourism/get-image/${photo}" alt="${photo}">`;
  });
  html += "</div>";
  return html;
}
...
function generateDocumentationHTML(documentation) {
  ...
  documentation.forEach(doc => {
    html += `<p><a href="/tourism/get-image/${doc}" target="_blank">${doc}</a></p>`;
  });
  return html;
}
...
function generateReportHTML(report) {
  ...
  report.forEach(report => {
    html += `<p><a href="/tourism/get-image/${report}" target="_blank">${report}</a></p>`;
  });
  return html;
}
function generatePresentationHTML(presentations) {
  ...
  presentations.forEach(presentation => {
    html += `<p><a href="/tourism/get-image/${presentation}" target="_blank">${presentation}</a></p>`;
  });
  return html;
}
function generateOtherHTML(other) {
  ...
  other.forEach(other => {
    html += `<p><a href="/tourism/get-image/${other}" target="_blank">${other}</a></p>`;
  });
  return html;
}
  ...
  db.query('SELECT * FROM Tourism WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
      if (err) {
          ...
          if (result.length === 0) {
              ...
              db.query('DELETE FROM Tour_Photographs WHERE project_id = ?', [projectId], (err, result) => {
                  if (err) {
                      console.error('Error deleting photographs:', err);
                      res.status(500).json({ error: 'Internal Server Error' });
                  } else {
                      db.query('DELETE FROM Tour_Documentation WHERE project_id = ?', [projectId], (err, result) => {
                          if (err) {
                              console.error('Error deleting documentation:', err);
                              res.status(500).json({ error: 'Internal Server Error' });
                          } else {
                              db.query('DELETE FROM Tour_Report WHERE project_id = ?', [projectId], (err, result) => {
                                  if (err) {
                                      console.error('Error deleting report:', err);
                                      res.status(500).json({ error: 'Internal Server Error' });
                                  } else {
                                      db.query('DELETE FROM Tour_Presentation WHERE project_id = ?', [projectId], (err, result) => {
                                          if (err) {
                                              console.error('Error deleting presentation:', err);
                                              res.status(500).json({ error: 'Internal Server Error' });
                                          } else {
                                              db.query('DELETE FROM Tour_Other WHERE project_id = ?', [projectId], (err, result) => {
                                                  if (err) {
                                                      console.error('Error deleting other files:', err);
                                                      res.status(500).json({ error: 'Internal Server Error' });
                                                  } else {
                                                      db.query('DELETE FROM Tourism WHERE project_id = ?', [projectId], (err, result) => {
                                                          if (err) {
...
router.post('/update', upload.fields([
  ...
  const other = req.files['other'] ? req.files['other'] : [];
   ...
   const generateNewFilename = (projectId, originalFilename) => {
    ...
    return `Tour${projectId}_${filenameWithoutExtension}${extension}`;
  };
   photographs.forEach(photo => {
    const oldPath = photo.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', generateNewFilename(projectId, photo.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, photo.originalname);

    db.query(`INSERT INTO Tour_Photographs (project_id, photograph) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  documentation.forEach(doc => {
    const oldPath = doc.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', generateNewFilename(projectId, doc.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, doc.originalname);

    db.query(`INSERT INTO Tour_Documentation (project_id, documentation) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  report.forEach(file => {
    ...
    const newPath = path.join(__dirname, '..', 'uploads', 'Tourism', generateNewFilename(projectId, file.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, file.originalname);

    db.query(`INSERT INTO Tour_Report (project_id, report) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  presentation.forEach(file => {
    ...
    db.query(`INSERT INTO Tour_Presentation (project_id, presentation) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  other.forEach(file => {
    ...
    db.query(`INSERT INTO Tour_Other (project_id, other) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
