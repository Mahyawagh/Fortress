    ...
    db.query('SELECT * FROM Industrial WHERE project_id = ?', [projectId], (err, result) => {
        if (err) {
            ...
            if (result.length > 0) {
                ...
                htmlContent += `<p><strong>Photographs:</strong> <a href="/manaindustrial/display_project?projectId=${project.project_id}" target="_blank">View Photographs</a></p>`;
                htmlContent += `<p><strong>Documentation:</strong> <a href="/manaindustrial/display_documentation?projectId=${project.project_id}" target="_blank">View Documentation</a></p>`;
                htmlContent += `<p><strong>Reports:</strong> <a href="/manaindustrial/display_report?projectId=${project.project_id}" target="_blank">View Report</a></p>`;
                htmlContent += `<p><strong>Presentation:</strong> <a href="/manaindustrial/display_presentation?projectId=${project.project_id}" target="_blank">View Presentation</a></p>`;
                htmlContent += `<p><strong>Other:</strong> <a href="/manaindustrial/display_other?projectId=${project.project_id}" target="_blank">View Other </a></p>`;
...
router.get('/project_list', (req, res) => {
    db.query('SELECT * FROM Industrial', (err, result) => {
        if (err) {
...
router.get('/search', (req, res) => {
    ...
    if (!isNaN(searchInput)) {
        ...
        db.query('SELECT * FROM Industrial WHERE project_id = ? OR year = ?', [projectId, projectId], (err, projectsResult) => {
            if (err) {
        ...
        db.query('SELECT * FROM Industrial WHERE project_name LIKE ?', [searchTerm], (err, projectsResult) => {
            if (err) {
                console.error('Error searching projects:', err);
                return res.status(500).json({ error: 'Internal Server Error' });
            }
...
function searchFiles(searchTerm, callback) {
    const sql = `
        SELECT project_id, 'photograph' AS type, photograph AS filename FROM Ind_Photographs WHERE photograph LIKE ? 
        UNION
        SELECT project_id, 'documentation' AS type, documentation AS filename FROM Ind_Documentation WHERE documentation LIKE ?
        UNION
        SELECT project_id, 'report' AS type, report AS filename FROM Ind_Report WHERE report LIKE ?
        UNION
        SELECT project_id, 'presentation' AS type, presentation AS filename FROM Ind_Presentation WHERE presentation LIKE ?
        UNION
        SELECT project_id, 'other' AS type, other AS filename FROM Ind_Other WHERE other LIKE ?
    `;
...
function generatePhotographsHTML(photographs) {
    ...
    photographs.forEach(photo => {
      html += `<img src="/manaindustrial/get-image/${photo}" alt="${photo}">`;
    });
    html += "</div>";
    return html;
  }
...
function generateDocumentationHTML(documentation) {
    ...
    documentation.forEach(doc => {
        html += `<p><a href="/manaindustrial/get-image/${doc}" target="_blank">${doc}</a></p>`;
    });
    return html;
}
...
function generateReportHTML(report) {
    ...
    report.forEach(report => {
        html += `<p><a href="/manaindustrial/get-image/${report}" target="_blank">${report}</a></p>`;
    });
    return html;
}
function generatePresentationHTML(presentation) {
    ...
    presentation.forEach(presentation => {
        html += `<p><a href="/manaindustrial/get-image/${presentation}" target="_blank">${presentation}</a></p>`;
    });
    return html;
}
...
function generateOtherHTML(other) {
    ...
    other.forEach(other => {
        html += `<p><a href="/manaindustrial/get-image/${other}" target="_blank">${other}</a></p>`;
    });
    return html;
}
