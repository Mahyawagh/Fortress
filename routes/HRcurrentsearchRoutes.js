    ...
    db.query('SELECT * FROM HRcurrent WHERE emp_id = ?', [empId], (err, result) => {
      if (err) {
        ...
        if (result.length > 0) {
          ...
          htmlContent += `<p><strong>Documentation:</strong> <a href="/HRcurrentsearch/display_documentation?empId=${project.emp_id}" target="_blank">View Documentation</a></p>`;
...
router.get('/project_list', (req, res) => {
  db.query('SELECT * FROM Hrcurrent', (err, result) => {
      if (err) {
...
router.get('/search', (req, res) => {
  ...
  if (!isNaN(searchInput)) {
      ...
      db.query('SELECT * FROM Hrcurrent WHERE emp_id = ? OR year = ?', [projectId, projectId], (err, projectsResult) => {
          if (err) {
      ...
      db.query('SELECT * FROM Hrcurrent WHERE EMployee_name LIKE ?', [searchTerm], (err, projectsResult) => {
          if (err) {
              console.error('Error searching projects:', err);
              return res.status(500).json({ error: 'Internal Server Error' });
          }
...
function searchFiles(searchTerm, callback) {
  const sql = `
      SELECT emp_id, 'documentation' AS type, documentation AS filename FROM Current_Documentation WHERE documentation LIKE ?
...
router.get('/display_documentation', (req, res) => {
 ...
 function generateDocumentationHTML(documentation) {
    ...
    documentation.forEach(doc => {
      html += `<p><a href="/HRcurrentsearch/get-image/${doc}" target="_blank">${doc}</a></p>`;
    });
    return html;
  }
