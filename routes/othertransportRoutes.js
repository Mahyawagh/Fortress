...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS OtherTransport (
  ...
  const createOTDocumentationTableQuery = `CREATE TABLE IF NOT EXISTS OT_Documentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ref_id INT,
    Documentation BLOB,
    FOREIGN KEY (ref_id) REFERENCES OtherTransport(ref_id)
  )`;
  db.query(createOTDocumentationTableQuery, err => {
    if (err) throw err;
    console.log('OT_Documentation table created');
  });
});

const storage = multer.diskStorage({
  destination: function(req, file, cb) {
    cb(null, 'uploads/Others/OtherTransport');
  },
  filename: function(req, file, cb) {
    ...
    const modifiedFilename = `OT${refId}_${Date.now()}_${file.fieldname}${extension}`; // Generate modified filename
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'Others','OtherTransport')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'Others', 'othertransport.html'));
});
...
router.post('/upload', upload.fields([
  ...
  const formattedName = name ? `OT_${name}` : ''; 
  const sql = `INSERT INTO OtherTransport (Name, year) VALUES (?, ?)`;
  db.query(sql, [formattedName, year], (err, result) => {
    if (err) {
      ...
      documentation.forEach(doc => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Others','OtherTransport', `OT${refId}_${doc.originalname}`);
        fs.renameSync(oldPath, newPath);

        // Insert documentation file name into Trans_Documentation table
        const insertDocumentationQuery = `INSERT INTO OT_Documentation (ref_id, Documentation) VALUES (?, ?)`;
        db.query(insertDocumentationQuery, [refId, `OT${refId}_${doc.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT c.ref_id, c.Name, c.year, d.Documentation FROM OtherTransport c LEFT JOIN OT_Documentation d ON c.ref_id = d.ref_id ', (err, result) => {
    if (err) {
...
router.get('/get-image/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, '..', 'uploads', 'Others','OtherTransport', filename);
...
router.get('/display', (req, res) => {
  ...
  db.query('SELECT * FROM OtherTransport WHERE ref_id = ?', [refId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Documentation:</strong> <a href="/OtherTransport/display_documentation?refId=${project.ref_id}" target="_blank">View Documentation</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM OtherTransport WHERE ref_id = ? OR Name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
router.get('/display_documentation', (req, res) => {
  ...
  db.query('SELECT Documentation FROM OT_Documentation WHERE ref_id = ?', [refId], (err, result) => {
    if (err) {
...
function generateDocumentationHTML(documentation) {
  ...
  documentation.forEach(doc => {
    html += `<p><a href="/OtherTransport/get-image/${doc}" target="_blank">${doc}</a></p>`;
  });
  return html;
}
...
router.post("/delete", (req, res) => {
  ...
  db.query(
    "SELECT * FROM OtherTransport WHERE ref_id = ? OR Name = ?",
    ...
    (err, result) => {
      ...
      db.query("DELETE FROM OT_Documentation WHERE ref_id = ?", [refId], (err, result) => {
        if (err) {
          console.error("Error deleting documentation:", err);
          return res.status(500).json({ error: "Internal Server Error" });
        }

        db.query("DELETE FROM OtherTransport WHERE ref_id = ?", [refId], (err, result) => {
          if (err) {
...
router.post('/update', update.fields([
  ...
  db.query('SELECT * FROM OtherTransport WHERE ref_id = ? OR Name = ?', [refId, refId], (err, result) => {
    if (err) {
    ...
    newDocumentation.forEach(doc => {
      ...
      // Insert new documentation file name into OT_Documentation table
      const insertDocumentationQuery = `INSERT INTO OT_Documentation (ref_id, Documentation) VALUES (?, ?)`;
      db.query(insertDocumentationQuery, [refId, `OT${refId}_${doc.originalname}`], (err, result) => {
        if (err) {
          console.error(err);
