...
router.get('/vertical', (req, res) => {
    const query = "SELECT id, name, email, password, status FROM verticalsFortress";
    connection.query(query, (err, results) => {
        if (err) {
            console.error(err);
            res.status(500).send('Error retrieving vertical details');
        } else {
...
router.put('/user_verticals/toggleStatus/:userId/:currentStatus', (req, res) => {
    console.log("hiii")
    const { userId, currentStatus } = req.params;
    const newStatus = currentStatus === 'Active' ? 'Inactive' : 'Active';
    const query = `UPDATE verticalsFortress SET status = ? WHERE id = ?`;
...
router.post('/vertical/add', (req, res) => {
    const { name, email, password } = req.body;
    const query = `INSERT INTO verticalsFortress (name, email, password, status) VALUES (?, ?, ?, 'Active')`;
...
router.delete('/vertical/delete', (req, res) => {
    const { name, email } = req.body;
    const query = `DELETE FROM verticalsFortress WHERE name = ? OR email = ?`;
...
router.put('/vertical/update', (req, res) => {
    ...
    const query = `UPDATE verticalsFortress SET ${updateFields} WHERE name = ? OR email = ?`;
