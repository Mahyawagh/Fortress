// //this code is working and display project list and click on project its open
// // managementenvironmentRoutes.js
...
//                 htmlContent += `<p><strong>Photographs:</strong> <a href="/managementenvironment/display_project?projectId=${project.project_id}" target="_blank">View Photographs</a></p>`;
//                 htmlContent += `<p><strong>Documentation:</strong> <a href="/managementenvironment/display_documentation?projectId=${project.project_id}" target="_blank">View Documentation</a></p>`;
//                 htmlContent += `<p><strong>Reports:</strong> <a href="/managementenvironment/display_report?projectId=${project.project_id}" target="_blank">View Report</a></p>`;
//                 htmlContent += `<p><strong>Presentation:</strong> <a href="/managementenvironment/display_presentation?projectId=${project.project_id}" target="_blank">View Presentation</a></p>`;
//                 htmlContent += `<p><strong>Other:</strong> <a href="/managementenvironment/display_other?projectId=${project.project_id}" target="_blank">View Other </a></p>`;
...
//     db.query('SELECT * FROM Environment WHERE project_id = ? OR project_name LIKE ? OR year = ?', [searchInput, `%${searchInput}%`, searchInput], (err, result) => {
//         if (err) {
//             console.error('Error searching project:', err);
//             res.status(500).json({ error: 'Internal Server Error' });
//         } else {
//             if (result.length > 0) {
//                 // If projects found, send JSON response with project details
//                 res.status(200).json(result);
//             } else {
//                 // If projects not found, send JSON response with 404 status code
...
//       html += `<img src="/managementenvironment/get-image/${photo}" alt="${photo}">`;
//     });
//     html += "</div>";
//     return html;
//   }
...
//         html += `<p><a href="/managementenvironment/get-image/${doc}" target="_blank">${doc}</a></p>`;
//     });
//     return html;
// }
...
//         html += `<p><a href="/managementenvironment/get-image/${report}" target="_blank">${report}</a></p>`;
//     });
//     return html;
// }
...
//         html += `<p><a href="/managementenvironment/get-image/${presentation}" target="_blank">${presentation}</a></p>`;
//     });
//     return html;
// }
...
//     other.forEach(other => {
//         html += `<p><a href="/managementenvironment/get-image/${other}" target="_blank">${other}</a></p>`;
//     });
//     return html;
// }
...
router.get('/search', (req, res) => {
    ...
    if (!isNaN(searchInput)) {
        ...
        db.query('SELECT * FROM Environment WHERE project_id = ? OR year = ?', [projectId, projectId], (err, projectsResult) => {
            if (err) {
        ...
        db.query('SELECT * FROM Environment WHERE project_name LIKE ?', [searchTerm], (err, projectsResult) => {
            if (err) {
                console.error('Error searching projects:', err);
                return res.status(500).json({ error: 'Internal Server Error' });
            }
...
function searchFiles(searchTerm, callback) {
    const sql = `
        SELECT project_id, 'photograph' AS type, photograph AS filename FROM Env_Photographs WHERE photograph LIKE ? 
        UNION
        SELECT project_id, 'documentation' AS type, documentation AS filename FROM Env_Documentation WHERE documentation LIKE ?
        UNION
        SELECT project_id, 'report' AS type, report AS filename FROM Env_Report WHERE report LIKE ?
        UNION
        SELECT project_id, 'presentation' AS type, presentation AS filename FROM Env_Presentation WHERE presentation LIKE ?
        UNION
        SELECT project_id, 'other' AS type, other AS filename FROM Env_Other WHERE other LIKE ?
    `;
