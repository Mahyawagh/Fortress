    ...
    db.query('SELECT * FROM BDTour WHERE project_id = ?', [projectId], (err, result) => {
      if (err) {
        ...
        if (result.length > 0) {
          ...
          htmlContent += `<p><strong>Tender Sight:</strong> <a href="/BDtourverticals/display_tendersight?projectId=${project.project_id}" target="_blank">View Tender Sight</a></p>`;
          htmlContent += `<p><strong>Tentative Letter:</strong> <a href="/BDtourverticals/display_tentativeletter?projectId=${project.project_id}"target="_blank">View Tentative Letters</a></p>`;
          htmlContent += `<p><strong>Bids Financial:</strong> <a href="/BDtourverticals/display_bidsfinancial?projectId=${project.project_id}"target="_blank">View Bids Financial Report </a></p>`;
          htmlContent += `<p><strong>Bids Technical:</strong> <a href="/BDtourverticals/display_bidstechnical?projectId=${project.project_id}"target="_blank">View Bids Technical Report</a></p>`;
...
router.get('/display_tendersight', (req, res) => {
  ...
  function generateTendersightHTML(tendersight) {
      ...
      tendersight.forEach(tendersight => {
          html += `<p><a href="/BDtourverticals/get-image/${tendersight}" target="_blank">${tendersight}</a></p>`;
      });
      return html;
  }
  ...
  function generateTentativeletterHTML(tentativeletter) {
      ...
      tentativeletter.forEach(tentativeletter => {
          html += `<p><a href="/BDtourverticals/get-image/${tentativeletter}" target="_blank">${tentativeletter}</a></p>`;
      });
      return html;
  }
  ...
  function generateBidsfinancialHTML(bidsfinancial) {
      ...
      bidsfinancial.forEach(bidsfinancial => {
          html += `<p><a href="/BDtourverticals/get-image/${bidsfinancial}" target="_blank">${bidsfinancial}</a></p>`;
      });
      return html;
  }
  ...
  function generateBidstechnicalHTML(bidstechnical) {
      ...
      bidstechnical.forEach(bidstechnical => {
          html += `<p><a href="/BDtourverticals/get-image/${bidstechnical}" target="_blank">${bidstechnical}</a></p>`;
      });
      return html;
  }
