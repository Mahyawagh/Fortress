...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS Transport (
  ...
  const createPhotographsTableQuery = `CREATE TABLE IF NOT EXISTS Trans_Photographs (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    photograph BLOB,
    FOREIGN KEY (project_id) REFERENCES Transport(project_id)
  )`;
  db.query(createPhotographsTableQuery, err => {
    if (err) throw err;
    console.log('Trans_Photographs table created');
  });

  const createDocumentationTableQuery = `CREATE TABLE IF NOT EXISTS Trans_Documentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    documentation BLOB,
    FOREIGN KEY (project_id) REFERENCES Transport(project_id)
  ...
  db.query(createDocumentationTableQuery, err => {
    if (err) throw err;
    console.log('Trans_Documentation table created');
  });
  const createReportTableQuery = `CREATE TABLE IF NOT EXISTS Trans_Report (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    report BLOB,
    FOREIGN KEY (project_id) REFERENCES Transport(project_id)
  ...
  db.query(createReportTableQuery, err => {
    if (err) throw err;
    console.log('Trans_Report table created');
  });
  const createPresentationTableQuery = `CREATE TABLE IF NOT EXISTS Trans_Presentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    presentation BLOB,
    FOREIGN KEY (project_id) REFERENCES Transport(project_id)
  ...
  db.query(createPresentationTableQuery, err => {
    if (err) throw err;
    console.log('Trans_Presentation table created');
  });
  const createOtherTableQuery = `CREATE TABLE IF NOT EXISTS Trans_Other (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    other BLOB,
    FOREIGN KEY (project_id) REFERENCES Transport(project_id)
  ...
  db.query(createOtherTableQuery, err => {
    if (err) throw err;
    console.log('Trans_Other table created');
  });
...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    ...
    const modifiedFilename = `Trans${projectId}_${Date.now()}_${file.fieldname}${extension}`; // Generate modified filename
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'Transportation')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'Transportation', 'transportation.html'));
});
...
router.post('/upload', upload.fields([
  ...
  //const formattedProjectName = `Trans${project_name ? '_' + project_name : ''}`; // Update formatted project name
  const formattedProjectName = `Trans${project_name ? '_' + project_name : ''}`; // Update formatted project name
  const sql = `INSERT INTO Transport (project_name, year) VALUES (?, ?)`;
  db.query(sql, [formattedProjectName, year], (err, result) => {
    if (err) {
      ...
      const formattedProjectName = `Trans${projectId}_${project_name}`;
    //console.log(formattedProjectName); 
      ...
      photographs.forEach(photo => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', `Trans${projectId}_${photo.originalname}`);
        fs.renameSync(oldPath, newPath);

        // Insert photograph file name into Trans_Photographs table
        const insertPhotographQuery = `INSERT INTO Trans_Photographs (project_id, photograph) VALUES (?, ?)`;
        db.query(insertPhotographQuery, [projectId, `Trans${projectId}_${photo.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      documentation.forEach(doc => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', `Trans${projectId}_${doc.originalname}`);
        fs.renameSync(oldPath, newPath);

        // Insert documentation file name into Trans_Documentation table
        const insertDocumentationQuery = `INSERT INTO Trans_Documentation (project_id, documentation) VALUES (?, ?)`;
        db.query(insertDocumentationQuery, [projectId, `Trans${projectId}_${doc.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      report.forEach(report => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', `Trans${projectId}_${report.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertReportQuery = `INSERT INTO Trans_Report (project_id, report) VALUES (?, ?)`;
        db.query(insertReportQuery, [projectId, `Trans${projectId}_${report.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      presentation.forEach(presentationFile => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', `Trans${projectId}_${presentationFile.originalname}`);
        fs.renameSync(oldPath, newPath);

        // Insert presentation file name into Trans_Presentation table
        const insertPresentationQuery = `INSERT INTO Trans_Presentation (project_id, presentation) VALUES (?, ?)`;
        db.query(insertPresentationQuery, [projectId, `Trans${projectId}_${presentationFile.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      other.forEach(other => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', `Trans${projectId}_${other.originalname}`);
        fs.renameSync(oldPath, newPath);
        
        const insertOtherQuery = `INSERT INTO Trans_Other (project_id, other) VALUES (?, ?)`;
        db.query(insertOtherQuery, [projectId, `Trans${projectId}_${other.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT t.project_id, t.project_name, t.year, p.photograph, d.documentation, r.report, pr.presentation, o.other FROM Transport t LEFT JOIN Trans_Photographs p ON t.project_id = p.project_id LEFT JOIN Trans_Documentation d ON t.project_id = d.project_id LEFT JOIN Trans_Report r ON t.project_id = r.project_id LEFT JOIN Trans_Presentation pr ON t.project_id = pr.project_id LEFT JOIN Trans_Other o ON t.project_id = o.project_id', (err, result) => {
    if (err) {
...
function generateProjectsHTML(projects) {
  ...
  // Create a map to store photographs and documentation by project ID
  const projectMap = new Map();

  // // Group photographs and documentation by project ID
...
router.get('/get-image/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, '..', 'uploads', 'Transportation', filename);
...
router.get('/display', (req, res) => {
  ...
  db.query('SELECT * FROM Transport WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Photographs:</strong> <a href="/transportation/display_project?projectId=${project.project_id}" target="_blank">View Photographs</a></p>`;
        htmlContent += `<p><strong>Documentation:</strong> <a href="/transportation/display_documentation?projectId=${project.project_id}" target="_blank">View Documentation</a></p>`;
        htmlContent += `<p><strong>Report:</strong> <a href="/transportation/display_report?projectId=${project.project_id}"target="_blank">View Report</a></p>`;
        htmlContent += `<p><strong>Presentation:</strong> <a href="/transportation/display_presentation?projectId=${project.project_id}"target="_blank">View Presentation </a></p>`;
        htmlContent += `<p><strong>Other:</strong> <a href="/transportation/display_other?projectId=${project.project_id}"target="_blank">View Other</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM Transport WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
router.get('/display_project', (req, res) => {
  ...
  db.query('SELECT photograph FROM Trans_Photographs WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_documentation', (req, res) => {
  ...
  db.query('SELECT documentation FROM Trans_Documentation WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_report', (req, res) => {
  ...
  db.query('SELECT report FROM Trans_Report WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_presentation', (req, res) => {
  ...
  db.query('SELECT presentation FROM Trans_Presentation WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_other', (req, res) => {
  ...
  db.query('SELECT other FROM Trans_Other WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
function generatePhotographsHTML(photographs) {
  ...
  `;
  photographs.forEach(photo => {
    html += `<img src="/transportation/get-image/${photo}" alt="${photo}">`;
  });
  html += "</div>";
  return html;
}
...
function generateDocumentationHTML(documentation) {
  ...
  documentation.forEach(doc => {
    html += `<p><a href="/transportation/get-image/${doc}" target="_blank">${doc}</a></p>`;
  });
  return html;
}
...
function generateReportHTML(report) {
  ...
  report.forEach(report => {
    html += `<p><a href="/transportation/get-image/${report}" target="_blank">${report}</a></p>`;
  });
  return html;
}
function generatePresentationHTML(presentations) {
  ...
  presentations.forEach(presentation => {
    html += `<p><a href="/transportation/get-image/${presentation}" target="_blank">${presentation}</a></p>`;
  });
  return html;
}
function generateOtherHTML(other) {
  ...
  other.forEach(other => {
    html += `<p><a href="/transportation/get-image/${other}" target="_blank">${other}</a></p>`;
  });
  return html;
}
  ...
  db.query('SELECT * FROM Transport WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
      if (err) {
          ...
          if (result.length === 0) {
              ...
              db.query('DELETE FROM Trans_Photographs WHERE project_id = ?', [projectId], (err, result) => {
                  if (err) {
                      console.error('Error deleting photographs:', err);
                      res.status(500).json({ error: 'Internal Server Error' });
                  } else {
                      db.query('DELETE FROM Trans_Documentation WHERE project_id = ?', [projectId], (err, result) => {
                          if (err) {
                              console.error('Error deleting documentation:', err);
                              res.status(500).json({ error: 'Internal Server Error' });
                          } else {
                              db.query('DELETE FROM Trans_Report WHERE project_id = ?', [projectId], (err, result) => {
                                  if (err) {
                                      console.error('Error deleting report:', err);
                                      res.status(500).json({ error: 'Internal Server Error' });
                                  } else {
                                      db.query('DELETE FROM Trans_Presentation WHERE project_id = ?', [projectId], (err, result) => {
                                          if (err) {
                                              console.error('Error deleting presentation:', err);
                                              res.status(500).json({ error: 'Internal Server Error' });
                                          } else {
                                              db.query('DELETE FROM Trans_Other WHERE project_id = ?', [projectId], (err, result) => {
                                                  if (err) {
                                                      console.error('Error deleting other files:', err);
                                                      res.status(500).json({ error: 'Internal Server Error' });
                                                  } else {
                                                      // Finally, delete project from Transport table
                                                      db.query('DELETE FROM Transport WHERE project_id = ?', [projectId], (err, result) => {
                                                          if (err) {
...
router.post('/update', upload.fields([
  ...
  const other = req.files['other'] ? req.files['other'] : [];
   ...
   const generateNewFilename = (projectId, originalFilename) => {
    ...
    return `Trans${projectId}_${filenameWithoutExtension}${extension}`;
  };
   // Update photographs in Trans_Photographs table
   photographs.forEach(photo => {
    const oldPath = photo.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', generateNewFilename(projectId, photo.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, photo.originalname);

    db.query(`INSERT INTO Trans_Photographs (project_id, photograph) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  // // Update documentation in Trans_Documentation table
  documentation.forEach(doc => {
    const oldPath = doc.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', generateNewFilename(projectId, doc.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, doc.originalname);

    // Insert documentation file name into Trans_Documentation table
    db.query(`INSERT INTO Trans_Documentation (project_id, documentation) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  // Update report in Trans_Report table
  report.forEach(file => {
    const oldPath = file.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', generateNewFilename(projectId, file.originalname));
    fs.renameSync(oldPath, newPath);
    const newFilename = generateNewFilename(projectId, file.originalname);

    // Insert report file name into Trans_Report table
    db.query(`INSERT INTO Trans_Report (project_id, report) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  // Update presentation in Trans_Presentation table
  presentation.forEach(file => {
    ...
    // Insert presentation file name into Trans_Presentation table
    db.query(`INSERT INTO Trans_Presentation (project_id, presentation) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
  ...
  // Update other in Trans_Other table
  other.forEach(file => {
    const oldPath = file.path;
    const newPath = path.join(__dirname, '..', 'uploads', 'Transportation', generateNewFilename(projectId, file.originalname));
    fs.renameSync(oldPath, newPath);
    ...
    // Insert other file name into Trans_Other table
    db.query(`INSERT INTO Trans_Other (project_id, other) VALUES (?, ?)`, [projectId, newFilename], (err) => {
      if (err) {
