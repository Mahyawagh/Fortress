...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS BDUrban (
  ...
  db.query(createTableQuery, err => {
    if (err) throw err;
    console.log('BDUrban table created');
  });

  const createTendersightTableQuery = `CREATE TABLE IF NOT EXISTS BDUrban_Tendersight (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    tendersight BLOB,
    FOREIGN KEY (project_id) REFERENCES BDUrban(project_id)
  )`;
  db.query(createTendersightTableQuery, err => {
    if (err) throw err;
    console.log('BDUrban_TenderSight table created');
  });

  const createTentativeletterTableQuery = `CREATE TABLE IF NOT EXISTS BDUrban_Tentativeletter (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
  ...
  db.query(createTentativeletterTableQuery, err => {
    if (err) throw err;
    console.log('BDUrban_Tentativeletter table created');
  });
  const createBidsfinancialTableQuery = `CREATE TABLE IF NOT EXISTS BDUrban_Bidsfinancial (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    bidsfinancial BLOB,
    FOREIGN KEY (project_id) REFERENCES BDUrban(project_id)
  ...
  db.query(createBidsfinancialTableQuery, err => {
    if (err) throw err;
    console.log('BDUrban_BidsFinancial table created');
  });
  const createBidstechnicalTableQuery = `CREATE TABLE IF NOT EXISTS BDUrban_Bidstechnical (
    id INT AUTO_INCREMENT PRIMARY KEY,
    project_id INT,
    bidstechnical BLOB,
    FOREIGN KEY (project_id) REFERENCES BDUrban(project_id)
  ...
  db.query(createBidstechnicalTableQuery, err => {
    if (err) throw err;
    console.log('BDUrban_Bidstechnical table created');
  });
...
const storage = multer.diskStorage({
  destination: function(req, file, cb) {
    cb(null, 'uploads/BusinessDevelopment/BDUrban');
  },
  filename: function(req, file, cb) {
    ...
    const modifiedFilename = `BDUrban${projectId}_${Date.now()}_${file.fieldname}${extension}`; // Generate modified filename
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'BusinessDevelopment','BDUrban')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'BusinessDevelopment', 'urbaninfrabusiness.html'));
});
...
router.post('/upload', upload.fields([
  ...
  const formattedProjectName = `BDUrban${project_name ? '_' + project_name : ''}`; // Update formatted project name
  const sql = `INSERT INTO BDUrban (project_name, year) VALUES (?, ?)`;
  db.query(sql, [formattedProjectName, year], (err, result) => {
    if (err) {
      ...
      const formattedProjectName = `BDUrban${projectId}_${project_name}`;
  
      tendersight.forEach(tender => {
        const oldPath = tender.path;
        const extension = path.extname(tender.originalname);
        const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment','BDUrban', `BDUrban${projectId}_${tender.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertTendersightQuery = `INSERT INTO BDUrban_Tendersight (project_id, tendersight) VALUES (?, ?)`;
        db.query(insertTendersightQuery, [projectId, `BDUrban${projectId}_${tender.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      tentativeletter.forEach(tentative => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment','BDUrban', `BDUrban${projectId}_${tentative.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertTentativeletterQuery = `INSERT INTO BDUrban_Tentativeletter (project_id, tentativeletter) VALUES (?, ?)`;
        db.query(insertTentativeletterQuery, [projectId, `BDUrban${projectId}_${tentative.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      bidsfinancial.forEach(finance => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment', 'BDUrban',`BDUrban${projectId}_${finance.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertBidsfinancialQuery = `INSERT INTO BDUrban_Bidsfinancial (project_id, bidsfinancial) VALUES (?, ?)`;
        db.query(insertBidsfinancialQuery, [projectId, `BDUrban${projectId}_${finance.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
      ...
      bidstechnical.forEach(technical => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment', 'BDUrban',`BDUrban${projectId}_${technical.originalname}`);
        fs.renameSync(oldPath, newPath);
        
        const insertBidstechnicalQuery = `INSERT INTO BDUrban_Bidstechnical (project_id, bidstechnical) VALUES (?, ?)`;
        db.query(insertBidstechnicalQuery, [projectId, `BDUrban${projectId}_${technical.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT u.project_id, u.project_name, u.year, s.tendersight, l.tentativeletter, f.bidsfinancial, bt.bidstechnical FROM BDUrban u LEFT JOIN BDUrban_Tendersight s ON u.project_id = s.project_id LEFT JOIN BDUrban_Tentativeletter l ON u.project_id = l.project_id LEFT JOIN BDUrban_Bidsfinancial f ON u.project_id = f.project_id LEFT JOIN BDUrban_Bidstechnical bt ON u.project_id = bt.project_id ', (err, result) => {
    if (err) {
...
router.get('/get-image/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment','BDUrban', filename);
...
router.get('/display', (req, res) => {
  ...
  db.query('SELECT * FROM BDUrban WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Tender Sight:</strong> <a href="/BDUrban/display_tendersight?projectId=${project.project_id}" target="_blank">View Tender Sight</a></p>`;
        htmlContent += `<p><strong>Tentative Letter:</strong> <a href="/BDUrban/display_tentativeletter?projectId=${project.project_id}"target="_blank">View Tentative Letters</a></p>`;
        htmlContent += `<p><strong>Bids Financial:</strong> <a href="/BDUrban/display_bidsfinancial?projectId=${project.project_id}"target="_blank">View Bids Financial Report </a></p>`;
        htmlContent += `<p><strong>Bids Technical:</strong> <a href="/BDUrban/display_bidstechnical?projectId=${project.project_id}"target="_blank">View Bids Technical Report</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM BDUrban WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
router.get('/display_tendersight', (req, res) => {
  ...
  db.query('SELECT tendersight FROM BDUrban_Tendersight WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_tentativeletter', (req, res) => {
  ...
  db.query('SELECT tentativeletter FROM BDUrban_Tentativeletter WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_bidsfinancial', (req, res) => {
  ...
  db.query('SELECT bidsfinancial FROM BDUrban_Bidsfinancial WHERE project_id = ?', [projectId], (err, result) => {
    if (err) {
...
router.get('/display_bidstechnical', (req, res) => {
    const projectId = req.query.projectId;
    db.query('SELECT bidstechnical FROM BDUrban_Bidstechnical WHERE project_id = ?', [projectId], (err, result) => {
        if (err) {
...
function generateTendersightHTML(tendersight) {
    ...
    tendersight.forEach(tendersight => {
        html += `<p><a href="/BDUrban/get-image/${tendersight}" target="_blank">${tendersight}</a></p>`;
    });
    return html;
}
...
function generateTentativeletterHTML(tentativeletter) {
    ...
    tentativeletter.forEach(tentativeletter => {
        html += `<p><a href="/BDUrban/get-image/${tentativeletter}" target="_blank">${tentativeletter}</a></p>`;
    });
    return html;
}
...
function generateBidsfinancialHTML(bidsfinancial) {
    ...
    bidsfinancial.forEach(bidsfinancial => {
        html += `<p><a href="/BDUrban/get-image/${bidsfinancial}" target="_blank">${bidsfinancial}</a></p>`;
    });
    return html;
}
...
function generateBidstechnicalHTML(bidstechnical) {
    ...
    bidstechnical.forEach(bidstechnical => {
        html += `<p><a href="/BDUrban/get-image/${bidstechnical}" target="_blank">${bidstechnical}</a></p>`;
    });
    return html;
}
  ...
  db.query('SELECT * FROM BDUrban WHERE project_id = ? OR project_name = ?', [searchInput, searchInput], (err, result) => {
      if (err) {
          ...
          if (result.length === 0) {
              ...
              const projectId = result[0].project_id;
              db.query('DELETE FROM BDUrban_Tendersight WHERE project_id = ?', [projectId], (err, result) => {
                  if (err) {
                      console.error('Error deleting tendersight:', err);
                      res.status(500).json({ error: 'Internal Server Error' });
                  } else {
                      db.query('DELETE FROM BDUrban_Tentativeletter WHERE project_id = ?', [projectId], (err, result) => {
                          if (err) {
                              console.error('Error deleting tentativeletter:', err);
                              res.status(500).json({ error: 'Internal Server Error' });
                          } else {
                              db.query('DELETE FROM BDUrban_Bidsfinancial WHERE project_id = ?', [projectId], (err, result) => {
                                  if (err) {
                                      console.error('Error deleting bidsfinancial:', err);
                                      res.status(500).json({ error: 'Internal Server Error' });
                                  } else {
                                      db.query('DELETE FROM BDUrban_Bidstechnical WHERE project_id = ?', [projectId], (err, result) => {
                                          if (err) {
                                              console.error('Error deleting bidstechnical:', err);
                                              res.status(500).json({ error: 'Internal Server Error' });
                                          } else {
                                              db.query('DELETE FROM BDUrban WHERE project_id = ?', [projectId], (err, result) => {
                                                  if (err) {
...
const bidstechnical = req.files['bidstechnical'] ? req.files['bidstechnical'] : [];
 ...
 const generateNewFilename = (projectId, originalFilename) => {
  ...
  return `BDUrban${projectId}_${filenameWithoutExtension}${extension}`;
};
 tendersight.forEach(tender => {
  const oldPath = tender.path;
  const newPath = path.join(__dirname, '..', 'uploads','BusinessDevelopment' ,'BDUrban', generateNewFilename(projectId, tender.originalname));
  fs.renameSync(oldPath, newPath);
  const newFilename = generateNewFilename(projectId, tender.originalname);

  db.query(`INSERT INTO BDUrban_Tendersight (project_id, tendersight) VALUES (?, ?)`, [projectId, newFilename], (err) => {
    if (err) {
...
tentativeletter.forEach(tentative => {
  const oldPath = tentative.path;
  const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment' ,'BDUrban',generateNewFilename(projectId, tentative.originalname));
  fs.renameSync(oldPath, newPath);
  const newFilename = generateNewFilename(projectId, tentative.originalname);

  db.query(`INSERT INTO BDUrban_Tentativeletter (project_id, tentativeletter) VALUES (?, ?)`, [projectId, newFilename], (err) => {
    if (err) {
...
bidsfinancial.forEach(bid => {
  ...
  const newPath = path.join(__dirname, '..', 'uploads', 'BusinessDevelopment' ,'BDUrban',generateNewFilename(projectId, bid.originalname));
  fs.renameSync(oldPath, newPath);
  const newFilename = generateNewFilename(projectId, bid.originalname);
  db.query(`INSERT INTO BDUrban_Bidsfinancial (project_id, bidsfinancial) VALUES (?, ?)`, [projectId, newFilename], (err) => {
    if (err) {
...
bidstechnical.forEach(bid => {
  ...
  fs.renameSync(oldPath, newPath);
  const newFilename = generateNewFilename(projectId, bid.originalname);
  db.query(`INSERT INTO BDUrban_Bidstechnical (project_id, bidstechnical) VALUES (?, ?)`, [projectId, newFilename], (err) => {
    if (err) {
