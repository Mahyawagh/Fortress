...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS StandEnv (
  ...
  const createOTDocumentationTableQuery = `CREATE TABLE IF NOT EXISTS Standenv_Documentation (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ref_id INT,
    Documentation BLOB,
    FOREIGN KEY (ref_id) REFERENCES StandEnv(ref_id)
  )`;
  db.query(createOTDocumentationTableQuery, err => {
    if (err) throw err;
    console.log('Standenv_Documentation table created');
  });
});

const storage = multer.diskStorage({
  destination: function(req, file, cb) {
    cb(null, 'uploads/Standard/StandEnv');
  },
  filename: function(req, file, cb) {
    ...
    const modifiedFilename = `SEnv${refId}_${Date.now()}_${file.fieldname}${extension}`; // Generate modified filename
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'Standard','StandEnv')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'Standard', 'standardenvironment.html'));
});
...
router.post('/upload', upload.fields([
  ...
  const formattedName = name ? `SEnv_${name}` : ''; 
  const sql = `INSERT INTO StandEnv (Name, year) VALUES (?, ?)`;
  db.query(sql, [formattedName, year], (err, result) => {
    if (err) {
      ...
      documentation.forEach(doc => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Standard','StandEnv', `SEnv${refId}_${doc.originalname}`);
        fs.renameSync(oldPath, newPath);

        const insertDocumentationQuery = `INSERT INTO Standenv_Documentation (ref_id, Documentation) VALUES (?, ?)`;
        db.query(insertDocumentationQuery, [refId, `SEnv${refId}_${doc.originalname}`], (err, result) => {
          if (err) {
            console.error(err);
            res.status(500).json({ error: 'Internal Server Error' });
          }
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT c.ref_id, c.Name, c.year, d.Documentation FROM StandEnv c LEFT JOIN Standenv_Documentation d ON c.ref_id = d.ref_id ', (err, result) => {
    if (err) {
...
router.get('/get-image/:filename', (req, res) => {
  ...
  const filePath = path.join(__dirname, '..', 'uploads', 'Standard','StandEnv', filename);
...
router.get('/display', (req, res) => {
  ...
  db.query('SELECT * FROM StandEnv WHERE ref_id = ?', [refId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Documentation:</strong> <a href="/StandEnv/display_documentation?refId=${project.ref_id}" target="_blank">View Documentation</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM StandEnv WHERE ref_id = ? OR Name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
router.get('/display_documentation', (req, res) => {
  ...
  db.query('SELECT Documentation FROM Standenv_Documentation WHERE ref_id = ?', [refId], (err, result) => {
    if (err) {
...
function generateDocumentationHTML(documentation) {
  ...
  documentation.forEach(doc => {
    html += `<p><a href="/StandEnv/get-image/${doc}" target="_blank">${doc}</a></p>`;
  });
  return html;
}
...
router.post("/delete", (req, res) => {
  ...
  db.query(
    "SELECT * FROM StandEnv WHERE ref_id = ? OR Name = ?",
    ...
    (err, result) => {
      ...
      db.query("DELETE FROM Standenv_Documentation WHERE ref_id = ?", [refId], (err, result) => {
        if (err) {
          console.error("Error deleting documentation:", err);
          return res.status(500).json({ error: "Internal Server Error" });
        }

        db.query("DELETE FROM StandEnv WHERE ref_id = ?", [refId], (err, result) => {
          if (err) {
...
router.post('/update', update.fields([
  ...
  db.query('SELECT * FROM StandEnv WHERE ref_id = ? OR Name = ?', [refId, refId], (err, result) => {
    if (err) {
    ...
    newDocumentation.forEach(doc => {
      ...
      // Insert new documentation file name into Standenv_Documentation table
      const insertDocumentationQuery = `INSERT INTO Standenv_Documentation (ref_id, Documentation) VALUES (?, ?)`;
      db.query(insertDocumentationQuery, [refId, `SEnv${refId}_${doc.originalname}`], (err, result) => {
        if (err) {
          console.error(err);
