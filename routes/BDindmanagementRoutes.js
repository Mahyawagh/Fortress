    ...
    db.query('SELECT * FROM BDInd WHERE project_id = ?', [projectId], (err, result) => {
      if (err) {
        ...
        if (result.length > 0) {
          ...
          htmlContent += `<p><strong>Tender Sight:</strong> <a href="/BDindmanagement/display_tendersight?projectId=${project.project_id}" target="_blank">View Tender Sight</a></p>`;
          htmlContent += `<p><strong>Tentative Letter:</strong> <a href="/BDindmanagement/display_tentativeletter?projectId=${project.project_id}"target="_blank">View Tentative Letters</a></p>`;
          htmlContent += `<p><strong>Bids Financial:</strong> <a href="/BDindmanagement/display_bidsfinancial?projectId=${project.project_id}"target="_blank">View Bids Financial Report </a></p>`;
          htmlContent += `<p><strong>Bids Technical:</strong> <a href="/BDindmanagement/display_bidstechnical?projectId=${project.project_id}"target="_blank">View Bids Technical Report</a></p>`;
...
router.get('/project_list', (req, res) => {
    db.query('SELECT * FROM BDInd', (err, result) => {
        if (err) {
...
router.get('/search', (req, res) => {
  ...
  if (!isNaN(searchInput)) {
      ...
      db.query('SELECT * FROM BDInd WHERE project_id = ? OR year = ?', [projectId, projectId], (err, projectsResult) => {
          if (err) {
      ...
      db.query('SELECT * FROM BDInd WHERE project_name LIKE ?', [searchTerm], (err, projectsResult) => {
          if (err) {
              console.error('Error searching projects:', err);
              return res.status(500).json({ error: 'Internal Server Error' });
          }
...
function searchFiles(searchTerm, callback) {
  const sql = `
      SELECT project_id, 'tendersight' AS type, tendersight AS filename FROM BDInd_Tendersight WHERE tendersight LIKE ? 
      UNION
      SELECT project_id, 'tentativeletter' AS type, tentativeletter AS filename FROM BDInd_Tentativeletter WHERE tentativeletter LIKE ?
      UNION
      SELECT project_id, 'bidsfinancial' AS type, bidsfinancial AS filename FROM BDInd_Bidsfinancial WHERE bidsfinancial LIKE ?
      UNION
      SELECT project_id, 'bidstechnical' AS type, bidstechnical AS filename FROM BDInd_Bidstechnical WHERE bidstechnical LIKE ?
...
router.get('/display_tendersight', (req, res) => {
  ...
  function generateTendersightHTML(tendersight) {
      ...
      tendersight.forEach(tendersight => {
          html += `<p><a href="/BDindmanagement/get-image/${tendersight}" target="_blank">${tendersight}</a></p>`;
      });
      return html;
  }
  ...
  function generateTentativeletterHTML(tentativeletter) {
      ...
      tentativeletter.forEach(tentativeletter => {
          html += `<p><a href="/BDindmanagement/get-image/${tentativeletter}" target="_blank">${tentativeletter}</a></p>`;
      });
      return html;
  }
  ...
  function generateBidsfinancialHTML(bidsfinancial) {
      ...
      bidsfinancial.forEach(bidsfinancial => {
          html += `<p><a href="/BDindmanagement/get-image/${bidsfinancial}" target="_blank">${bidsfinancial}</a></p>`;
      });
      return html;
  }
  ...
  function generateBidstechnicalHTML(bidstechnical) {
      ...
      bidstechnical.forEach(bidstechnical => {
          html += `<p><a href="/BDindmanagement/get-image/${bidstechnical}" target="_blank">${bidstechnical}</a></p>`;
      });
      return html;
  }
