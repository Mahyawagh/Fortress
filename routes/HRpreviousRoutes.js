...
db.connect(err => {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS HRprevious (
  ...
  db.query(createTableQuery, err => {
    if (err) throw err;
    console.log('HRprevious table created');
  });

  const createEmpDocumentationTableQuery = `CREATE TABLE IF NOT EXISTS Previous_Documentation (
  ...
  db.query(createEmpDocumentationTableQuery, err => {
    if (err) throw err;
    console.log('Previous_Documentation table created');
  });
});

const storage = multer.diskStorage({
  destination: function(req, file, cb) {
    cb(null, 'uploads/Humanresource/PreviousEmployee');
  },
...
router.use('/uploads', express.static(path.join(__dirname, '..', 'uploads', 'Humanresource','PreviousEmployee')));

router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'Humanresource', 'previous_employee.html'));
});
...
router.post('/upload', upload.fields([
  ...
  const sql = `INSERT INTO HRprevious (Employee_name, year) VALUES (?, ?)`;
  db.query(sql, [formattedEmployeeName, year], (err, result) => {
    if (err) {
      ...
      documentation.forEach(doc => {
        ...
        const newPath = path.join(__dirname, '..', 'uploads', 'Humanresource','PreviousEmployee', `Emp${empId}_${doc.originalname}`);
        fs.renameSync(oldPath, newPath);

        // Insert documentation file name into Trans_Documentation table
        const insertDocumentationQuery = `INSERT INTO Previous_Documentation (emp_id, Documentation) VALUES (?, ?)`;
...
router.get('/projectsHTML', (req, res) => {
  db.query('SELECT p.emp_id, p.Employee_name, p.year, d.Documentation FROM HRprevious p LEFT JOIN Previous_Documentation d ON p.emp_id = d.emp_id ', (err, result) => {
    if (err) {
  ...
  db.query('SELECT * FROM HRprevious WHERE emp_id = ?', [empId], (err, result) => {
    if (err) {
      ...
      if (result.length > 0) {
        ...
        htmlContent += `<p><strong>Documentation:</strong> <a href="/hrprevious/display_documentation?empId=${project.emp_id}" target="_blank">View Documentation</a></p>`;
...
router.get('/search', (req, res) => {
  ...
  db.query('SELECT * FROM HRprevious WHERE emp_id = ? OR Employee_name = ?', [searchInput, searchInput], (err, result) => {
    if (err) {
...
function generateDocumentationHTML(documentation) {
  ...
  documentation.forEach(doc => {
    html += `<p><a href="/hrprevious/get-image/${doc}" target="_blank">${doc}</a></p>`;
  });
  return html;
}
...
router.post("/delete", (req, res) => {
  ...
  db.query(
    "SELECT * FROM HRprevious WHERE emp_id = ? OR Employee_name = ?",
    ...
    (err, result) => {
      ...
      db.query("DELETE FROM Previous_Documentation WHERE emp_id = ?", [empId], (err, result) => {
        if (err) {
        ...
        db.query("DELETE FROM HRprevious WHERE emp_id = ?", [empId], (err, result) => {
          if (err) {
...
router.post('/update', update.fields([
  ...
  db.query('SELECT * FROM HRprevious WHERE emp_id = ? OR Employee_name = ?', [empId, empId], (err, result) => {
    if (err) {
