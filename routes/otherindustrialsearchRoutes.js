    ...
    db.query('SELECT * FROM OtherIndustrial WHERE ref_id = ?', [refId], (err, result) => {
        if (err) {
            ...
            if (result.length > 0) {
                ...
                htmlContent += `<p><strong>Documentation:</strong> <a href="/otherindustrialsearch/display_documentation?refId=${project.ref_id}" target="_blank">View Documentation</a></p>`;
...
router.get('/project_list', (req, res) => {
    db.query('SELECT * FROM OtherIndustrial', (err, result) => {
        if (err) {
...
router.get('/search', (req, res) => {
    ...
    if (!isNaN(searchInput)) {
        ...
        db.query('SELECT * FROM OtherIndustrial WHERE ref_id = ? OR year = ?', [refId, refId], (err, projectsResult) => {
            if (err) {
        ...
        db.query('SELECT * FROM OtherIndustrial WHERE Name LIKE ?', [searchTerm], (err, projectsResult) => {
            if (err) {
                console.error('Error searching projects:', err);
                return res.status(500).json({ error: 'Internal Server Error' });
            }
...
function searchFiles(searchTerm, callback) {
    const sql = `
        SELECT ref_id, 'documentation' AS type, documentation AS filename FROM OI_Documentation WHERE documentation LIKE ? 
...
function generateDocumentationHTML(documentation) {
    ...
    documentation.forEach(doc => {
        html += `<p><a href="/otherindustrialsearch/get-image/${doc}" target="_blank">${doc}</a></p>`;
    });
    return html;
}
